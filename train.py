...
parser = argparse.ArgumentParser(
    description="""This script trains YOLOv10 model.""",
    formatter_class=argparse.ArgumentDefaultsHelpFormatter
)
parser.add_argument("-m", "--model", required=True, help="Relative path to model file. Specify 'yolov10{n/s/m/l/x}.pt' to load pretrained model from ultralytics."\
                    " It will be automatically downloaded during first use. Specify 'yolov10n.yaml' to build new model.")
...
if MODEL.endswith('.yaml'):
    if TW_MODEL != None :
        ...
        run = YOLOv10(MODEL).load(TW_MODEL)
    else:
